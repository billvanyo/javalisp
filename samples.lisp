...
(defun subsets (l) (if (null l) '(()) ((lambda (s) (append s (mapcar (lambda (x) (cons (car l) x)) s))) (subsets (cdr l)))))
...
(defun mapappend (f l) (if (null l) nil (append (f (car l)) (mapappend f (cdr l)))))

; given an element x and a list l, return a list of all possible ways of inserting x somewhere in list l
(defun insert (x l) (if (null l) (cons (cons x nil) nil) (cons (cons x l) (mapcar (lambda (l2) (cons (car l) l2)) (insert x (cdr l))))))
...
(defun perms (l) (if (null l) '(()) (mapappend (lambda (p) (insert (car l) p)) (perms (cdr l)))))

; standard recursive factorial
(defun factorial (n) (if (zerop n) 1 (* n (factorial (1- n)))))

; length of a list
(defun length (l) (if (null l) 0 (1+ (length (cdr l)))))
